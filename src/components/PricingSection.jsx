...
export default function PricingSection({ onSelectPlan }) {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // Featured (middle) card also starts slightly scaled down
      const featuredCard = cards[1];
      if (featuredCard) {
        gsap.set(featuredCard, { opacity: 0, y: 30, scale: 0.96 });
      }

      // Staggered entrance triggered when section scrolls into view
      gsap.to(cards, {
        opacity: 1,
        y: 0,
        scale: 1,
